...
def test_criar_aluno_gera_matricula_e_id_por_curso():
    primeiro_ges = criar_aluno("Ana Silva", "ana@email.com", "GES")
    segundo_ges = criar_aluno("Bruno Lima", "bruno@email.com", "GES")
    primeiro_gec = criar_aluno("Carla Souza", "carla@email.com", "GEC")

    assert primeiro_ges.status_code == 200
    assert segundo_ges.status_code == 200
    assert primeiro_gec.status_code == 200

    assert primeiro_ges.json()["matricula"] == 1
    assert primeiro_ges.json()["id"] == "GES1"
    assert segundo_ges.json()["matricula"] == 2
    assert segundo_ges.json()["id"] == "GES2"
    assert primeiro_gec.json()["matricula"] == 1
    assert primeiro_gec.json()["id"] == "GEC1"
...
def test_atualizar_curso_gera_novo_id_sem_reutilizar_matricula():
